            ...
            <div id="chat-container" className={open ? "hidden fixed bottom-16 right-4 w-96" : "fixed bottom-16 right-4 w-96"} style={{ zIndex: 999 }}>
                <div className="bg-white shadow-md rounded-lg max-w-lg w-full">
                    <div className="p-4 border-b bg-gray-900 text-white rounded-t-lg flex justify-between items-center">
                        ...
                        <button onClick={() => setOpen(!open)} id="close-chat" className="text-black bg-white  hover:text-gray-400 focus:outline-none focus:text-gray-400">
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                            </svg>
                        </button>
                    </div>
                    <div id="chatbox" className="p-4 h-80 bg-green-400/[0.5] overflow-y-auto">
                        {chats.map((chat, index) => (
                            <div key={index} className={chat.role === "ai" ? "mb-2" : "mb-2 text-right"}>
                                <p className={chat.role === "ai" ? "bg-gray-900 text-white rounded-lg py-2 px-4 inline-block" : "bg-blue-500 text-white rounded-lg py-2 px-4 inline-block"}>{chat.m}</p>
                            </div>
                        ))}
                    </div>
                    <div className="p-4 border-t flex">
                        ...
                        <button onClick={getResponse} id="send-button" className="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-teal-300 to-lime-300 group-hover:from-teal-300 group-hover:to-lime-300 dark:text-white dark:hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-lime-800">
